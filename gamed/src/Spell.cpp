...
loadLua(script); //comment this line for no reload on the fly, better performance
    CORE_INFO("Spell from slot (onCast) %i", getSlot());

   try{
      script.lua.script("onCast()");
   ...
   if (castTime > 0 && !(flags & SPELL_FLAG_InstantCast)) {
      ...
      owner->getMap()->getGame()->notifySpellAnimation(owner, "lux_spell4.anm");
      state = STATE_CASTING;
...
void Spell::finishCasting() {
	...
	if(getSlot() < 4) {
        ...
        owner->getMap()->getGame()->notifySetCooldown(owner, getSlot(), cooldown.at(0), cooldown.at(0)); //testing
	}
}
