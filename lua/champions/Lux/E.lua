...
function finishCasting()
    ...
    local boltRange = Vector2:Mult(to,750)
    local boltCoords = Vector2:Add(current, boltRange)

    addProjectile(boltCoords.x, boltCoords.y)
end

function applyEffects()
	dealMagicalDamage(getEffectValue(0) + (0.75 * getOwner():getStats():getTotalAp()))
	addParticleTarget("Ezreal_arcaneshift_mis.troy", getTarget())
	destroyProjectile()
end